...
class CalibrationConfig(BaseModel):
    """Top-level configuration for the calibration script."""
    base_pipeline_config: FilePath = Field("config/track_config.json", description="Path to the base pipeline config file to use for calibration runs.")
    parameters_to_calibrate: Dict[str, List[Any]] = Field(..., description="A dictionary where keys are parameter names and values are lists of settings to test.")
    output_results_path: str = Field("annotations/calibration_results.csv", description="Path to save the output CSV with calibration results.")
